import { POS_FRACTION_DIGITS } from "./constants";
...
export class Pos {
  ...
  isInCenter(): boolean {
    const isXPointFive = (x: number) =>
      Number((x - 0.5).toFixed(POS_FRACTION_DIGITS)) === Math.floor(x);
    return isXPointFive(this.x) && isXPointFive(this.y);
  }

  calcFieldPos(): Pos[] {
    const isXPointFive = (x: number) =>
      Number((x - 0.5).toFixed(POS_FRACTION_DIGITS)) === Math.floor(x); // is like x.5 (x % 0.5 == 0 && !isInteger(x))

    if (isXPointFive(this.y) && Number.isInteger(this.x)) {
      return [new Pos(this.x, Math.floor(this.y)), new Pos(this.x - 1, Math.floor(this.y))];
    } else if (isXPointFive(this.x) && Number.isInteger(this.y)) {
      return [new Pos(Math.floor(this.x), this.y), new Pos(Math.floor(this.x), this.y - 1)];
    } else {
  ...
  public static addDirToPos(pos: Pos, dir: Dir, dirMultiplier: number = 1) {
    return new Pos(
      Number((pos.x + dir.x * dirMultiplier).toFixed(POS_FRACTION_DIGITS)),
      Number((pos.y + dir.y * dirMultiplier).toFixed(POS_FRACTION_DIGITS)),
    );
  }
}
